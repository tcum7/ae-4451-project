...
class Engine:
    ...
    def fan(self, P, T):
        ...
        work_total = Cp*(T_o_fan - T)          # per *total* air mass
 
        # Convert fan work to per core mass (needed by fan turbine)
        work_core = (1 + self.Beta) * work_total
    ...
    # ------------------------------------------------------------
    # CORE TURBINE (drives compressor)
    # ------------------------------------------------------------
    def turbine(self, P_o, T_o, work_c):
        gamma = 1.33
        Cp = gamma*self.R/(gamma - 1)
 
        # turbine mass flow = (1 - b + f) m_core
        T_o_turb = T_o - work_c / (Cp*(1 - self.b + self.f))
        Tr = T_o_turb / T_o
 
        exp = 0.92*(gamma - 1)/gamma
        Pr = Tr**(1/exp)
    ...
    def fanturbine(self, P_o, T_o, work_f):
        ...
        # Fan turbine mass flow = (1 + f) m_core
        T_o_turb = T_o - work_f / (Cp*(1 + self.f))
        Tr = T_o_turb / T_o
    ...
    # COMBINED NOZZLE
    # ------------------------------------------------------------
    def combinednozzle(self, P_o, T_o, P_fan, T_fan):
        T_mix = (T_o + self.Beta*T_o) / (1 + self.Beta)
 
        gamma = 1.37
        Cp = gamma*self.R/(gamma - 1)
 
        P_mix = 0.8*(P_o + self.Beta*P_o)/(1 + self.Beta)
 
        T_eideal = T_mix * (self.P_a/P_mix)**((gamma - 1)/gamma)
        T_e = T_mix - 0.95 * (T_mix - T_eideal)
    ...
    def run_cycle(self):
        ...
        # 2. Fan
        P13, T13, drag, W_fan_total, W_fan_core = self.fan(P2, T2)
